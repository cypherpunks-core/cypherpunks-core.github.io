...
在任何時候，資金都可以用下面的指令碼來支付：
```
0 <Alice's signature> <Bob's signature> 0
```
Lenny經過3個月後，Alice或Bob中的一個可以用以下指令碼支付資金：
```
...
現在，使用者總是能夠在沒有服務合作的情況下花費他們的資金，等待到期時間的到來。

### 支付通道

Jeremy Spilman風格的支付通道首先設定一個存款，由2-of-2的多簽名，tx1控制的存款，然後調整第二個交易tx2，將tx1的輸出用於支付者和收款者。在釋出tx1之前，建立一個退款交易tx3，以確保收款人是否可以清除付款人的存款。建立退款交易的過程目前易受交易延展性攻擊的影響，此外還要求付款人儲存退款。使用與雙因子錢包示例中相同的scriptPubKey形式解決了這兩個問題。

## 用於釋出資料的無信任支付

PayPub協議可以通過首先證明加密檔案包含所需資料，然後製作用於付款的scriptPubKeys來支付以無信任方式付款的資訊，以便花費它們顯示資料的加密金鑰。然而，現有的實現有一個重大缺陷：釋出者可以無限期地推遲金鑰的釋出。
...
證明犧牲一些有限的資源是各種密碼協議中的常見技術。已經提出將硬幣的犧牲證明為採礦費，作為犧牲可以指向的普遍公共物品，而不是簡單地摧毀硬幣。然而，這樣做並非微不足道，即使是最好的現有技術 - 宣佈 - 承諾 - 也會鼓勵礦業集中。`CHECKLOCKTIMEVERIFY`可用於建立任何人都可以花費的產出（因此，假設礦工的行為是理想的和理性的，那麼開採費），但只有在未來足夠遠的時間，大型礦工才能以折扣銷售犧牲品。
